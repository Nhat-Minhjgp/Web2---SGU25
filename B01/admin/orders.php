...
<head>
    ...
    <style>
        ...
        .track-modal-content { background: white; border-radius: 16px; width: 90%; max-width: 900px; max-height: 85vh; overflow-y: auto; animation: fadeIn 0.3s ease-out; }
        ...
        .action-btn { padding: 4px 10px; border-radius: 6px; font-size: 12px; font-weight: 500; display: inline-flex; align-items: center; gap: 4px; text-decoration: none; }
...
<div class="flex w-full min-h-[calc(100vh-70px)]">
    <aside class="w-64 bg-white shadow-lg hidden lg:block flex-shrink-0 border-r border-gray-100">
        ...
        <nav class="p-4 space-y-2">
            ...
            <a href="import.php" class="flex items-center gap-3 px-4 py-3 text-gray-600 rounded-lg hover:bg-gray-50 hover:text-primary transition"><i class="fas fa-arrow-down w-5"></i> Quản lý nhập hàng</a>
            ...
            <a href="orders.php" class="flex items-center gap-3 px-4 py-3 bg-gradient-custom text-white rounded-lg shadow-md"><i class="fas fa-receipt w-5"></i> Quản lý đơn hàng</a>
    ...
    <main class="flex-1 p-6 lg:p-8 overflow-x-hidden bg-gray-50">
        <div class="bg-white rounded-xl shadow-lg p-6 lg:p-8 min-h-full">
            <div class="flex justify-between items-center mb-6 pb-4 border-b">
                <h2 class="text-2xl font-bold text-gray-800"><i class="fas fa-receipt text-primary mr-2"></i>Quản lý đơn hàng</h2>
            </div>

            <?php if (isset($message)): ?>
                <div class="bg-green-50 border-l-4 border-green-500 text-green-700 p-4 mb-4 rounded"><i class="fas fa-check-circle mr-2"></i><?php echo $message; ?></div>
            <?php endif; ?>
            <?php if (isset($error)): ?>
                <div class="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 mb-4 rounded"><i class="fas fa-exclamation-circle mr-2"></i><?php echo $error; ?></div>
            <?php endif; ?>

            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <input type="text" id="searchOrder" placeholder="🔍 Tìm theo ID, tên khách, SĐT..." class="px-4 py-2 border border-gray-300 rounded-lg">
                <div class="flex gap-2"><input type="date" id="fromDate" class="px-4 py-2 border border-gray-300 rounded-lg flex-1"><span class="self-center">-</span><input type="date" id="toDate" class="px-4 py-2 border border-gray-300 rounded-lg flex-1"></div>
                <select id="statusFilter" class="px-4 py-2 border border-gray-300 rounded-lg"><option value="">Tất cả trạng thái</option><option value="0">Chờ xử lý</option><option value="1">Đã xác nhận</option><option value="2">Đã giao</option><option value="3">Đã hủy</option></select>
                <select id="sortWard" class="px-4 py-2 border border-gray-300 rounded-lg"><option value="">Sắp xếp theo phường</option><option value="asc">A → Z</option><option value="desc">Z → A</option></select>
            </div>

            <div class="overflow-x-auto border border-gray-200 rounded-xl">
                <table class="w-full min-w-[1000px]" id="ordersTable">
                    <thead class="bg-gradient-custom text-white">
                        <tr><th class="px-4 py-3 text-left">ID</th><th class="px-4 py-3 text-left">Mã đơn</th><th class="px-4 py-3 text-left">Khách hàng</th><th class="px-4 py-3 text-left">SĐT</th><th class="px-4 py-3 text-left">Phường</th><th class="px-4 py-3 text-right">Tổng tiền</th><th class="px-4 py-3 text-left">Ngày đặt</th><th class="px-4 py-3 text-left">Trạng thái</th><th class="px-4 py-3 text-center">Thao tác</th></tr>
                    </thead>
                    <tbody id="ordersTableBody">
                        <?php foreach ($orders as $order): $status = intval($order['TrangThai']); $isLocked = ($status == 2 || $status == 3); $order_code = "ORD" . str_pad($order['DonHang_id'], 5, '0', STR_PAD_LEFT); ?>
                        <tr class="hover:bg-gray-50 transition" data-ward="<?php echo htmlspecialchars($order['Quan'] ?? ''); ?>">
                            ...
                            <td class="px-4 py-3"><?php echo htmlspecialchars($order['customer_name'] ?? $order['Ten_nguoi_nhan'] ?? 'Khách lẻ'); ?></td>
                            <td class="px-4 py-3"><?php echo htmlspecialchars($order['customer_phone'] ?? $order['SDT_nhan'] ?? ''); ?></td>
                            ...
                            <td class="px-4 py-3"><?php echo date('d/m/Y H:i', strtotime($order['NgayDat'])); ?></td>
                            <td class="px-4 py-3"><span class="status-badge status-<?php echo $status; ?>"><?php echo $status == 0 ? '⏳ Chờ xử lý' : ($status == 1 ? '✓ Đã xác nhận' : ($status == 2 ? '✅ Đã giao' : '✗ Đã hủy')); ?></span></td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <button onclick="viewOrderDetail(<?php echo $order['DonHang_id']; ?>)" class="text-blue-500 hover:text-blue-700 p-1" title="Xem chi tiết"><i class="fas fa-eye"></i></button>
                                    <?php if ($isLocked): ?>
                                        <span class="action-btn btn-disabled"><i class="fas fa-lock"></i> Đã khóa</span>
                                    <?php elseif ($status == 0): ?>
                                        <a href="?action=update&id=<?php echo $order['DonHang_id']; ?>&status=1" class="action-btn btn-confirm" onclick="return confirm('Xác nhận đơn hàng? Hành động này sẽ tạo phiếu xuất và trừ tồn kho!')"><i class="fas fa-check"></i> Xác nhận</a>
                                        <a href="?action=update&id=<?php echo $order['DonHang_id']; ?>&status=3" class="action-btn btn-cancel" onclick="return confirm('Hủy đơn hàng?')"><i class="fas fa-times"></i> Hủy</a>
                                    <?php elseif ($status == 1): ?>
                                        <a href="?action=update&id=<?php echo $order['DonHang_id']; ?>&status=2" class="action-btn btn-deliver" onclick="return confirm('Xác nhận đã giao hàng?')"><i class="fas fa-check-double"></i> Đã giao</a>
                                        <a href="?action=update&id=<?php echo $order['DonHang_id']; ?>&status=3" class="action-btn btn-cancel" onclick="return confirm('Hủy đơn hàng?')"><i class="fas fa-times"></i> Hủy</a>
                                    <?php endif; ?>
                                </div>
                            </td>
...
<div id="trackModal" class="modal"><div class="track-modal-content"><div class="bg-gradient-custom text-white px-6 py-4 rounded-t-xl flex justify-between"><h3 class="text-xl font-semibold"><i class="fas fa-truck mr-2"></i>Chi tiết đơn hàng</h3><button onclick="closeModal()" class="text-white text-2xl">&times;</button></div><div class="p-6" id="trackModalContent"><div class="text-center py-12"><i class="fas fa-spinner fa-spin text-3xl"></i><p>Đang tải...</p></div></div><div class="px-6 py-4 border-t flex justify-end"><button onclick="closeModal()" class="px-4 py-2 bg-gray-500 text-white rounded-lg">Đóng</button></div></div></div>
...
function filterByDate(){let f=document.getElementById('fromDate').value,t=document.getElementById('toDate').value;document.querySelectorAll('#ordersTableBody tr').forEach(r=>{let d=r.cells[6]?.textContent.split(' ')[0];if(!d)return;let [day,month,year]=d.split('/');let rd=new Date(`${year}-${month}-${day}`);let show=true;if(f&&rd<new Date(f))show=false;if(t&&rd>new Date(t))show=false;r.style.display=show?'':'none';});}
function filterByStatus(){let s=document.getElementById('statusFilter').value;document.querySelectorAll('#ordersTableBody tr').forEach(r=>{let sp=r.cells[7]?.querySelector('span');let val='';if(sp?.textContent.includes('Chờ xử lý'))val='0';else if(sp?.textContent.includes('Đã xác nhận'))val='1';else if(sp?.textContent.includes('Đã giao'))val='2';else if(sp?.textContent.includes('Đã hủy'))val='3';r.style.display=(s===''||val===s)?'':'none';});}
function sortByWard(){let o=document.getElementById('sortWard').value;if(!o)return;let tbody=document.getElementById('ordersTableBody');let rows=Array.from(tbody.querySelectorAll('tr'));rows.sort((a,b)=>{let wa=a.getAttribute('data-ward')||'',wb=b.getAttribute('data-ward')||'';return o==='asc'?wa.localeCompare(wb,'vi'):wb.localeCompare(wa,'vi');});rows.forEach(r=>tbody.appendChild(r));}
