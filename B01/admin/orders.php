...
$sql = "SELECT d.*, u.Ho_ten as customer_name, u.SDT as customer_phone,
        dc.Ten_nguoi_nhan, dc.SDT_nhan, dc.Quan, dc.Tinh_thanhpho
        FROM donhang d
...
<body class="bg-gray-100 font-sans min-h-screen">
        ...
        <main class="flex-1 p-8">
            <div class="bg-white rounded-xl shadow-lg p-6 animate-fadeIn">
                ...
                <div class="overflow-x-auto border border-gray-200 rounded-xl">
                    <table class="w-full min-w-[900px]" id="ordersTable">
                        ...
                        <tbody id="ordersTableBody" class="divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition" data-ward="<?php echo htmlspecialchars($order['Quan'] ?? ''); ?>">
                                <td class="px-4 py-3 font-mono text-sm">#ORD<?php echo str_pad($order['DonHang_id'], 5, '0', STR_PAD_LEFT); ?></td>
                                ...
                                <td class="px-4 py-3">
                                    <span class="px-3 py-1 rounded-full text-xs font-medium 
                                        <?php echo $status == 0 ? 'bg-yellow-100 text-yellow-700' : ($status == 1 ? 'bg-blue-100 text-blue-700' : ($status == 2 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700')); ?>">
                                        <?php echo $status == 0 ? '⏳ Chờ xử lý' : ($status == 1 ? '✓ Đã xác nhận' : ($status == 2 ? '✅ Đã giao' : '✗ Đã hủy')); ?>
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2 flex-wrap">
                                        <button onclick="viewOrderDetail(<?php echo $order['DonHang_id']; ?>)" class="text-blue-500 hover:text-blue-700 p-1 rounded-lg hover:bg-blue-50 transition" title="Xem chi tiết">
                                        ...
                                        <?php elseif ($status == 0): ?>
                                            <a href="?action=update&id=<?php echo $order['DonHang_id']; ?>&status=1" class="inline-flex items-center gap-1 px-3 py-1 bg-blue-500 text-white rounded-lg text-xs hover:bg-blue-600 transition" onclick="return confirm('Xác nhận đơn hàng?')">
                                                <i class="fas fa-check"></i> Xác nhận
                                            </a>
                                            <a href="?action=update&id=<?php echo $order['DonHang_id']; ?>&status=3" class="inline-flex items-center gap-1 px-3 py-1 bg-red-500 text-white rounded-lg text-xs hover:bg-red-600 transition" onclick="return confirm('Hủy đơn hàng?')">
                                                <i class="fas fa-times"></i> Hủy
                                            </a>
                                        <?php elseif ($status == 1): ?>
                                            <a href="?action=update&id=<?php echo $order['DonHang_id']; ?>&status=2" class="inline-flex items-center gap-1 px-3 py-1 bg-green-500 text-white rounded-lg text-xs hover:bg-green-600 transition" onclick="return confirm('Xác nhận đã giao hàng?')">
    ...
    <div id="orderDetailModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-xl w-full max-w-2xl max-h-[80vh] overflow-y-auto animate-fadeIn">
            ...
            <div class="px-6 py-4 border-t border-gray-200 flex justify-end sticky bottom-0 bg-white">
                <button onclick="closeModal()" class="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition">Đóng</button>
            </div>
        </div>
    </div>
    ...
    <script>
        function viewOrderDetail(orderId) {
            ...
            content.innerHTML = '<div class="text-center py-8"><i class="fas fa-spinner fa-spin text-2xl text-indigo-600"></i><p class="text-gray-500 mt-2">Đang tải...</p></div>';
                                ...
                                <tr class="border-b">
                                    <td class="p-2">
                                        <div class="flex items-center gap-2">
                                            ${item.image_url ? `<img src="../${item.image_url}" class="w-10 h-10 object-cover rounded-lg border">` : '<div class="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center"><i class="fas fa-image text-gray-400"></i></div>'}
        ...
        function filterByDate() {
            ...
            document.querySelectorAll('#ordersTableBody tr').forEach(row => {
                const date = row.cells[5]?.textContent.trim().split(' ')[0];
                if (!date) return;
        ...
        window.onclick = function(event) {
            if (event.target.classList.contains('fixed') && event.target.id === 'orderDetailModal') {
                closeModal();
            }
        }
    </script>
