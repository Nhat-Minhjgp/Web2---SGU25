...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_product'])) {
    ...
    if ($nguong_canh_bao < 0)
        $nguong_canh_bao = 0;
    if ($nguong_canh_bao > 9999)
        $nguong_canh_bao = 9999;
    ...
    if (!$valid) {
        ...
        try {
            $stmt = $conn->prepare("INSERT INTO sanpham (TenSP, Danhmuc_id, NCC_id, Ma_thuonghieu, MoTa, image_url, PhanTramLoiNhuan, CanhBaoTon, TrangThai, SoLuongTon, TaoNgay) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, 0, NOW())");
            $stmt->bind_param("siiisddi", $tenSP, $danhmuc_id, $ncc_id, $ma_thuonghieu, $mota, $imageUrl, $phanTramLoiNhuan, $nguong_canh_bao);
            $stmt->execute();
            $stmt->close();
    ...
    <div class="flex w-full min-h-[calc(100vh-70px)]">
        ...
        <main class="flex-1 p-6 lg:p-8 overflow-x-hidden bg-gray-50">
            <form method="POST" enctype="multipart/form-data"
                class="bg-white rounded-xl shadow-lg p-6 lg:p-8 max-w-3xl mx-auto" id="addProductForm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="tenSP" id="product_name" required
                    ...
                    <!-- Danh mục -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Danh mục <span
                                class="text-red-500">*</span></label>
                        <select name="danhmuc_id" id="category" required
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition bg-white">
                            <option value="">-- Chọn danh mục --</option>
                            <?php while ($dm = $danhmucs->fetch_assoc()): ?>
                                <option value="<?php echo $dm['Danhmuc_id']; ?>">
                    ...
                    <div>
                        ...
                        <select name="thuonghieu_id" id="brand"
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition bg-white">
                            <option value="0">-- Chọn thương hiệu --</option>
                            <?php while ($th = $thuonghieus->fetch_assoc()): ?>
                                <option value="<?php echo $th['Ma_thuonghieu']; ?>">
                    ...
                    <!-- Nhà cung cấp -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Nhà cung cấp <span
                                class="text-red-500">*</span></label>
                        <select name="ncc_id" id="supplier" required
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition bg-white">
                            <option value="">-- Chọn NCC --</option>
                            <?php while ($ncc = $nhacungcaps->fetch_assoc()): ?>
                                <option value="<?php echo $ncc['NCC_id']; ?>">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">% Lợi nhuận dự kiến <span
                                class="text-red-500">*</span></label>
                        <input type="number" name="phan_tram_loi" id="profit" min="0" max="200" value="20"
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            ...
                            <span class="info-badge"
                                style="background:#e5e7eb;color:#374151;padding:2px 8px;border-radius:20px;font-size:11px;margin-left:6px;">Tùy
                                chọn</span>
                        </label>
                        <input type="number" name="nguong_canh_bao" id="threshold" min="0" max="9999" value="10"
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition">
                        <p class="text-xs text-gray-500 mt-1">* Cảnh báo khi tồn kho ≤ giá trị này (mặc định: 10)</p>
                        <div class="error-text" id="threshold-error">Giá trị phải từ 0 đến 9999</div>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="mota" id="description" rows="3"
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition"
                ...
                <div class="mt-8 flex gap-3 justify-end">
                    ...
                    <button type="submit" name="save_product" id="submitBtn"
    ...
    <script>
        ...
        function validateThreshold() {
            ...
            if (isNaN(value) || value < 0 || value > 9999) {
                showError(input, 'Giá trị phải từ 0 đến 9999', 'threshold-error');
                return false;
            }
            hideError(input, 'threshold-error');
            return true;
        }

        // Chặn nhập ký tự không hợp lệ
        document.getElementById('threshold')?.addEventListener('keydown', function (e) {
