...
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case 'price_asc': $sort_order = "s.GiaBan ASC"; break;
        case 'price_desc': $sort_order = "s.GiaBan DESC"; break;
        case 'name_asc': $sort_order = "s.TenSP ASC"; break;
        default: $sort_order = "s.SanPham_id DESC";
    }
}
...
.filter-btn { position: relative; cursor: pointer; transition: all 0.2s ease; display: block; width: 100%; }
.filter-btn input[type="checkbox"] { position: absolute; opacity: 0; width: 0; height: 0; }
.filter-btn .btn-content { display: flex; align-items: center; justify-content: space-between; padding: 10px 12px; background: #f9fafb; border: 2px solid #e5e7eb; border-radius: 8px; font-size: 13px; color: #374151; transition: all 0.2s ease; width: 100%; min-height: 44px; box-sizing: border-box; }
.filter-btn input[type="checkbox"]:checked + .btn-content { background: #fef2f2; border-color: #dc2626; color: #dc2626; font-weight: 500; }
.filter-btn:hover .btn-content { border-color: #dc2626; background: #fef2f2; }
.filter-btn .btn-label { display: flex; align-items: center; gap: 6px; flex: 1; min-width: 0; }
.filter-btn .btn-label span { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.filter-btn .btn-count { font-size: 11px; color: #6b7280; background: #e5e7eb; padding: 2px 6px; border-radius: 10px; margin-left: 6px; flex-shrink: 0; min-width: 24px; text-align: center; }
.filter-btn input[type="checkbox"]:checked + .btn-content .btn-count { background: #fecaca; color: #dc2626; }
.filter-btn .check-icon { opacity: 0; transform: scale(0.8); transition: all 0.2s ease; color: #dc2626; font-size: 14px; flex-shrink: 0; margin-left: 4px; }
.filter-btn input[type="checkbox"]:checked + .btn-content .check-icon { opacity: 1; transform: scale(1); }
.sort-dropdown { position: relative; }
.sort-select { appearance: none; padding: 8px 36px 8px 12px; border: 2px solid #e5e7eb; border-radius: 8px; font-size: 13px; background: white; cursor: pointer; transition: all 0.2s ease; }
.sort-select:focus { outline: none; border-color: #dc2626; }
.sort-dropdown::after { content: '\f107'; font-family: 'Font Awesome 6 Free'; font-weight: 900; position: absolute; right: 12px; top: 50%; transform: translateY(-50%); color: #6b7280; pointer-events: none; }
.filter-btn-grid { display: flex; flex-direction: column; gap: 8px; }
.filter-btn-grid .filter-btn { display: block; width: 100%; }
...
.user-menu {
    ...
    border: 1px solid #f3f4f6; min-width: 220px; z-index: 50;
...
.user-menu-item i { width: 18px; color: #6b7280; }
...
.role-badge-staff { background: #dc2626; }

/* Search SQL Injection Warning */
.search-sql-warning { 
    background-color: #fef2f2; border: 2px solid #dc2626; 
    color: #dc2626; padding: 8px 12px; border-radius: 6px; 
    margin-top: 8px; font-size: 12px; display: none; 
}
.search-sql-warning i { margin-right: 6px; }
</style>
...
<div id="mobile-filter-drawer" class="fixed inset-y-0 right-0 z-50 w-full lg:hidden bg-white shadow-2xl closed flex flex-col h-full">
    <div class="p-4 border-b flex items-center justify-between bg-white sticky top-0 z-10">
        <h3 class="font-bold text-lg flex items-center"><i class="fas fa-sliders-h mr-2 text-red-600"></i> Bộ lọc sản phẩm</h3>
        <button id="close-mobile-filter" class="p-2 hover:bg-gray-100 rounded-full transition"><i class="fas fa-times text-gray-500 text-lg"></i></button>
    </div>
    ...
    <div class="p-4 border-t bg-white sticky bottom-0 z-10 flex gap-3">
        <button type="button" id="reset-mobile-filter" class="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-800 py-3 rounded-lg font-medium transition"><i class="fas fa-undo mr-2"></i>Reset</button>
        <button type="button" id="apply-mobile-filter" class="flex-1 bg-red-600 hover:bg-red-700 text-white py-3 rounded-lg font-medium transition shadow-lg shadow-red-200"><i class="fas fa-check mr-2"></i>Áp dụng</button>
    </div>
</div>
...
<!-- Mobile Menu Toggle -->
<div class="md:hidden"><button class="menu-toggle p-2"><img src="../img/icons/menu.svg" class="fas fa-bars text-2xl"></button></div>
...
<img src="../img/icons/menu.svg" class="w-5 h-5 mr-2" alt="menu"><span>Danh mục</span>
</button>
<div id="mega-menu-dropdown" class="absolute left-0 top-full mt-2 w-[900px] bg-white rounded-lg shadow-xl hidden z-50">
<div class="flex p-4"><div class="w-64 border-r border-gray-200 pr-4">
...
<div class="w-8 h-8 flex-shrink-0 mr-3"><img src="https://nvbplay.vn/wp-content/uploads/2024/10/jogging-No.svg" alt="Giày" class="w-full h-full"></div>
<div><p class="font-bold">Giày</p><p class="text-xs text-gray-500">Giày thể thao tối ưu hoá vận động</p></div>
</div>
...
<div class="w-6 h-6 flex-shrink-0 mr-3"><img src="https://nvbplay.vn/wp-content/uploads/2024/10/healthcare-No.svg" alt="Chăm sóc sức khoẻ" class="w-full h-full"></div><div><p class="font-bold">Chăm sóc sức khoẻ</p></div>
</a>
...
<div class="w-6 h-6 flex-shrink-0 mr-3"><img src="https://nvbplay.vn/wp-content/uploads/2024/10/customer-service-No.svg" alt="Dịch vụ" class="w-full h-full"></div><div><p class="font-bold">Dịch vụ</p></div>
</a>
<div class="icon-box-menu p-3 mb-1 cursor-pointer hover:bg-gray-50 transition flex items-start" data-menu="news">
<div class="w-6 h-6 flex-shrink-0 mr-3"><img src="https://nvbplay.vn/wp-content/uploads/2024/10/news-No.svg" alt="Tin Tức" class="w-full h-full"></div>
<div><p class="font-bold">Tin Tức</p><p class="text-xs text-gray-500">Xu hướng mới, sự kiện hot, giảm giá sốc!</p></div>
</div>
</div><div class="flex-1 pl-4"><div id="content-badminton" class="menu-content"></div><div id="content-pickleball" class="menu-content hidden"></div><div id="content-giay" class="menu-content hidden"></div></div>
</div></div></li>
...
<i class="fas fa-map-marker-alt mr-1"></i><span class="shipping-address text-sm"><span class="text">Chọn địa chỉ</span></span>
</a>
</div>
...
<button class="search-toggle p-2"><i class="fas fa-search text-gray-700 hover:text-red-600"></i></button>
</div>
...
<span class="text-sm font-medium text-gray-700"><?php echo htmlspecialchars($user_info['username']); ?></span>
...
<a href="./my-account.php" class="user-menu-item"><i class="fas fa-user"></i><span>Tài khoản của tôi</span></a>
<a href="./my-account/orders.php" class="user-menu-item"><i class="fas fa-shopping-bag"></i><span>Đơn hàng</span></a>
<a href="./my-account/address-book.php" class="user-menu-item"><i class="fas fa-map-marker-alt"></i><span>Sổ địa chỉ</span></a>
<div class="user-menu-divider"></div>
<a href="../control/logout.php" class="user-menu-item logout"><i class="fas fa-sign-out-alt"></i><span>Đăng xuất</span></a>
</div>
<?php else: ?>
<!-- Chưa đăng nhập -->
<a href="./login.php" class="flex items-center text-gray-700 hover:text-red-600">
<i class="far fa-user text-xl"></i><span class="text-sm ml-1">Đăng nhập</span>
</a>
<?php endif; ?>
</div>
...
<?php if ($is_logged_in): ?>
<a href="./my-account.php" class="p-1"><img src="../img/icons/account.svg" class="w-6 h-6" alt="Account"></a>
<?php else: ?>
<a href="./login.php" class="p-1"><i class="far fa-user text-xl"></i></a>
<?php endif; ?>
<a href="./cart.php" class="relative p-1"><i class="fas fa-shopping-basket text-xl"></i><span class="absolute -top-1 -right-1 bg-red-600 text-white text-xs rounded-full h-4 w-4 flex items-center justify-center">0</span></a>
</div>
</div>
</div>
</div>
...
<a href="shop.php" class="text-sm text-red-600 hover:text-red-700 font-medium"><i class="fas fa-times-circle mr-1"></i>Xóa tất cả</a>
</div>
...
<button onclick="removeFilter('search')" class="w-5 h-5 bg-red-600 text-white rounded-full flex items-center justify-center hover:bg-red-700 transition"><i class="fas fa-times text-xs"></i></button>
</div>
<?php endif; ?>
...
<button onclick="removeSingleFilter('danhmuc', '<?php echo $slug; ?>')" class="w-5 h-5 bg-red-600 text-white rounded-full flex items-center justify-center hover:bg-red-700 transition"><i class="fas fa-times text-xs"></i></button>
</div>
<?php endforeach; ?>
<?php endif; ?>
...
<button onclick="removeSingleFilter('thuonghieu', '<?php echo $slug; ?>')" class="w-5 h-5 bg-red-600 text-white rounded-full flex items-center justify-center hover:bg-red-700 transition"><i class="fas fa-times text-xs"></i></button>
</div>
<?php endforeach; ?>
<?php endif; ?>
...
<button onclick="removeFilter('price')" class="w-5 h-5 bg-red-600 text-white rounded-full flex items-center justify-center hover:bg-red-700 transition"><i class="fas fa-times text-xs"></i></button>
</div>
<?php endif; ?>
</div>
</div>
...
<h3 class="font-bold text-lg flex items-center"><i class="fas fa-filter mr-2 text-red-600"></i>Bộ lọc tìm kiếm</h3>
<button type="button" id="reset-desktop-filter" class="text-sm text-red-600 hover:text-red-700 font-medium transition"><i class="fas fa-undo mr-1"></i>Reset</button>
</div>
</div>
<div class="p-5 space-y-4 max-h-[calc(100vh-200px)] overflow-y-auto custom-scrollbar" id="desktop-filter-content">
<!-- Tìm kiếm sản phẩm -->
<div class="border-b border-gray-100 pb-4">
<h4 class="font-semibold text-gray-800 mb-3 flex items-center"><i class="fas fa-search text-red-600 mr-2"></i>Tìm kiếm sản phẩm</h4>
<div class="relative">
<input type="text" id="search-input" class="w-full pl-4 pr-10 py-2.5 border-2 border-gray-200 rounded-lg text-sm focus:border-red-500 focus:outline-none transition" placeholder="Nhập tên sản phẩm..." value="<?php echo htmlspecialchars($search_keyword); ?>">
<?php if (!empty($search_keyword)): ?>
<button type="button" onclick="clearSearch()" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-red-600 transition p-1"><i class="fas fa-times"></i></button>
<?php endif; ?>
</div>
<!-- SQL Injection Warning -->
<div id="searchSqlWarning" class="search-sql-warning"><i class="fas fa-shield-alt"></i><span id="searchSqlMsg">Phát hiện ký tự không an toàn</span></div>
<p class="text-xs text-gray-500 mt-2"><i class="fas fa-info-circle mr-1"></i>Nhấn Enter hoặc nút "Áp dụng bộ lọc" để tìm</p>
</div>

<!-- Danh mục -->
<div class="border-b border-gray-100 pb-4">
<h4 class="font-semibold text-gray-800 mb-3 flex items-center"><i class="fas fa-folder text-red-600 mr-2"></i>Danh mục sản phẩm<span class="text-xs font-normal text-gray-500 ml-auto">(Chọn nhiều)</span></h4>
...
<input type="checkbox" name="danhmuc[]" value="<?php echo $category['slug']; ?>" <?php echo isActiveFilter('danhmuc', $category['slug']) ? 'checked' : ''; ?>>
<div class="btn-content">
<div class="btn-label"><i class="fas fa-circle text-[6px] <?php echo isActiveFilter('danhmuc', $category['slug']) ? 'text-red-600' : 'text-gray-300'; ?>"></i><span class="text-sm" title="<?php echo htmlspecialchars($category['Ten_danhmuc']); ?>"><?php echo htmlspecialchars($category['Ten_danhmuc']); ?></span></div>
<div class="flex items-center gap-1"><span class="btn-count"><?php echo $category['product_count']; ?></span><i class="fas fa-check-circle check-icon"></i></div>
</div>
</label>
<?php endwhile; ?>
</div>
...
<!-- Thương hiệu -->
<div class="border-b border-gray-100 pb-4">
<h4 class="font-semibold text-gray-800 mb-3 flex items-center"><i class="fas fa-tag text-red-600 mr-2"></i>Thương hiệu<span class="text-xs font-normal text-gray-500 ml-auto">(Chọn nhiều)</span></h4>
<div class="filter-btn-grid">
<?php foreach ($brands_list as $brand): ?>
<label class="filter-btn">
<input type="checkbox" name="thuonghieu[]" value="<?php echo $brand['slug']; ?>" <?php echo isActiveFilter('thuonghieu', $brand['slug']) ? 'checked' : ''; ?>>
<div class="btn-content">
<div class="btn-label"><i class="fas fa-circle text-[6px] <?php echo isActiveFilter('thuonghieu', $brand['slug']) ? 'text-red-600' : 'text-gray-300'; ?>"></i><span class="text-sm" title="<?php echo htmlspecialchars($brand['Ten_thuonghieu']); ?>"><?php echo htmlspecialchars($brand['Ten_thuonghieu']); ?></span></div>
<div class="flex items-center gap-1"><span class="btn-count"><?php echo $brand['product_count']; ?></span><i class="fas fa-check-circle check-icon"></i></div>
</div>
</label>
<?php endforeach; ?>
</div>
...
<!-- Khoảng giá -->
<div>
<h4 class="font-semibold text-gray-800 mb-3 flex items-center"><i class="fas fa-money-bill-wave text-red-600 mr-2"></i>Khoảng giá</h4>
<div class="flex items-center gap-3 mb-4">
<div class="flex-1"><label class="text-xs text-gray-500 mb-1 block">Từ</label><input type="text" id="price-min" class="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-right text-sm focus:border-red-500 focus:outline-none transition price-input" value="<?php echo number_format($min_price, 0, ',', '.'); ?>" placeholder="0₫" min="0"></div>
<span class="text-gray-400 pt-5">-</span>
<div class="flex-1"><label class="text-xs text-gray-500 mb-1 block">Đến</label><input type="text" id="price-max" class="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-right text-sm focus:border-red-500 focus:outline-none transition price-input" value="<?php echo number_format($max_price, 0, ',', '.'); ?>" placeholder="50.000.000₫" min="0" max="50000000"></div>
</div>
<button type="button" id="apply-filter" class="w-full bg-red-600 hover:bg-red-700 text-white py-3 rounded-lg text-sm font-semibold transition shadow-md shadow-red-200"><i class="fas fa-filter mr-2"></i>Áp dụng bộ lọc</button>
</div>
</div>
</div>
</div>
...
<div class="text-sm text-gray-600">Hiển thị <?php if ($total_products > 0): ?><span class="font-semibold text-gray-900"><?php echo $offset + 1; ?>-<?php echo min($offset + $limit, $total_products); ?></span><?php else: ?><span class="font-semibold text-gray-900">0</span><?php endif; ?> trong <span class="font-semibold text-gray-900"><?php echo $total_products; ?></span> kết quả <?php if (!empty($search_keyword)): ?><span class="text-red-600"> cho "<?php echo htmlspecialchars($search_keyword); ?>"</span><?php endif; ?></div>
...
<i class="fas fa-filter text-red-600"></i><span class="font-semibold">Bộ lọc sản phẩm</span>
...
<div class="absolute top-2 left-2 z-10"><span class="bg-gray-500 text-white text-xs px-2 py-1 rounded-full font-semibold">Hết hàng</span></div>
<?php endif; ?>
<div class="aspect-square bg-gray-100 flex items-center justify-center p-4">
<img src="..<?php echo htmlspecialchars($product['image_url']); ?>" alt="<?php echo htmlspecialchars($product['TenSP']); ?>" class="w-full h-full object-contain mix-blend-multiply group-hover:scale-110 transition-transform duration-300" onerror="this.src='../img/sanpham/placeholder.png'">
</div>
</div>
<div class="p-3">
<h3 class="font-medium text-sm mb-2 line-clamp-2 h-10 hover:text-red-600 transition"><?php echo htmlspecialchars($product['TenSP']); ?></h3>
<div class="flex items-center gap-2 flex-wrap">
<span class="text-red-600 font-bold"><?php echo formatPrice($product['GiaBan']); ?></span>
<?php if ($discount > 0): ?><span class="text-gray-400 text-xs line-through"><?php echo formatPrice($product['GiaNhapTB']); ?></span><?php endif; ?>
</div>
<?php if ($product['SoLuongTon'] < 10 && $product['SoLuongTon'] > 0): ?>
<div class="text-xs text-orange-500 mt-1 font-semibold"><i class="fas fa-exclamation-triangle mr-1"></i>Chỉ còn <?php echo $product['SoLuongTon']; ?> sản phẩm</div>
<?php endif; ?>
</div>
</a>
</div>
...
<a href="<?php echo buildFilterUrl(['page' => $page - 1]); ?>" class="w-10 h-10 flex items-center justify-center rounded-lg border-2 border-gray-200 bg-white hover:bg-gray-50 hover:border-red-300 transition <?php echo ($page <= 1) ? 'pointer-events-none opacity-50' : ''; ?>"><i class="fas fa-chevron-left text-sm"></i></a>
...
<a href="<?php echo buildFilterUrl(['page' => $i]); ?>" class="w-10 h-10 flex items-center justify-center rounded-lg border-2 border-gray-200 bg-white hover:bg-gray-50 hover:border-red-300 transition"><?php echo $i; ?></a>
<?php elseif ($i == $page - 3 || $i == $page + 3): ?>
<span class="w-10 h-10 flex items-center justify-center">...</span>
<?php endif; ?>
<?php endfor; ?>
<a href="<?php echo buildFilterUrl(['page' => $page + 1]); ?>" class="w-10 h-10 flex items-center justify-center rounded-lg border-2 border-gray-200 bg-white hover:bg-gray-50 hover:border-red-300 transition <?php echo ($page >= $total_pages) ? 'pointer-events-none opacity-50' : ''; ?>"><i class="fas fa-chevron-right text-sm"></i></a>
</div>
</div>
<?php endif; ?>
...
<p class="text-gray-500 mb-4"><?php if (!empty($search_keyword)): ?>Không có sản phẩm nào phù hợp với từ khóa "<?php echo htmlspecialchars($search_keyword); ?>".<?php else: ?>Không có sản phẩm nào phù hợp với bộ lọc của bạn.<?php endif; ?></p>
<a href="shop.php" class="inline-block px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition font-semibold shadow-md shadow-red-200"><i class="fas fa-times-circle mr-2"></i>Xóa bộ lọc</a>
</div>
<?php endif; ?>
</div>
</div>
...
<div class="pl-5"><h3 class="text-4xl font-bold mb-4">Boost<br>your power</h3>
...
<div><h3 class="text-xl font-bold mb-4">Thông tin khác</h3>
<ul class="space-y-2">
...
<div><h3 class="text-xl font-bold mb-4">Về chúng tôi</h3>
<ul class="space-y-3">
...
<div class="text-gray-500 text-sm mb-4 md:mb-0"><p>©2025 CÔNG TY CỔ PHẦN NVB PLAY</p></div>
...
<button class="close-menu p-2 hover:bg-gray-100 rounded-full transition"><i class="fas fa-times text-2xl text-gray-600"></i></button>
</div>
<div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
<?php if ($is_logged_in): ?>
<div class="flex items-center text-gray-700">
<div class="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center mr-3"><img src="../img/icons/account.svg" class="w-6 h-6" alt="Account"></div>
<div><div class="font-medium"><?php echo htmlspecialchars($user_info['username']); ?></div><span class="text-sm text-gray-500"><?php echo htmlspecialchars($user_info['email']); ?></span></div>
</div>
...
<div class="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center mr-3"><i class="far fa-user text-xl text-gray-600"></i></div>
<div><div class="font-medium">Tài khoản</div><span class="text-sm text-gray-500">Đăng nhập / Đăng ký</span></div>
</a>
<?php endif; ?>
</div>
