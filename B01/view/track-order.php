...
// ✅ FIX: Kiểm tra bước timeline đã hoàn thành dựa trên status integer
function isStepCompleted($currentStatus, $stepStatus)
{
    $current = (int) $currentStatus;
    $step = (int) $stepStatus;
    // Nếu đã hủy (3) thì chỉ hiển thị bước 0 là completed
    if ($current === 3)
        return $step === 0;
    return $current >= $step;
}
            ...
            <div class="bg-white rounded-xl shadow p-6 mb-6">
                <div class="flex items-center justify-between mb-6 pb-6 border-b">
                    <div>
                        ...
                        <p class="text-gray-600">Mã đơn: <span
                                class="font-mono font-bold">#<?php echo $order['DonHang_id']; ?></span></p>
                    </div>
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium">
                                    ...
                                    echo $methods[$order['PhuongThucTT']] ?? htmlspecialchars($order['PhuongThucTT']);
                                    ?>
                                </span>
                            </div>
                            <div class="flex justify-between">
                                ...
                                <tr>
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span
                                                class="text-sm font-medium"><?php echo htmlspecialchars($item['TenSP']); ?></span>
                                        </div>
                                    </td>
                ...
                <!-- Order Status Timeline (FIXED for integer status) -->
                <h3 class="font-semibold text-gray-900 mb-3"> Tiến trình đơn hàng</h3>
                <div class="bg-gray-50 rounded-lg p-6 mb-6">
                    <?php
                    $currentStatus = (int) ($order['TrangThai'] ?? 0);
                    $isCancelled = ($currentStatus === 3);
                    ?>

                    <!-- Step 0: Đặt hàng -->
                    <div
                        class="timeline-item <?php echo $isCancelled ? 'cancelled' : (isStepCompleted($currentStatus, 0) ? 'completed' : ''); ?>">
                    ...
                    <!-- Step 1: Xác nhận -->
                    <div
                        class="timeline-item mt-4 <?php echo $isCancelled ? '' : (isStepCompleted($currentStatus, 1) ? 'completed' : ''); ?>">
                        <div class="timeline-dot"></div>
                        <p class="font-medium text-gray-900"> Đã xác nhận</p>
                        <p class="text-sm text-gray-500">
                            <?php echo $currentStatus >= 1 ? 'Đang xử lý đơn hàng' : 'Chờ xác nhận'; ?></p>
                    </div>

                    <!-- Step 2: Giao hàng -->
                    <div
                        class="timeline-item mt-4 <?php echo $isCancelled ? '' : (isStepCompleted($currentStatus, 2) ? 'completed' : ''); ?>">
                        <div class="timeline-dot"></div>
                        <p class="font-medium text-gray-900"> Đang giao hàng</p>
                        <p class="text-sm text-gray-500">
                            <?php echo $currentStatus >= 2 ? 'Đơn hàng đang trên đường giao' : 'Chờ vận chuyển'; ?></p>
                    </div>

                    <!-- Step 3: Hoàn thành/Hủy -->
                    <div
                        class="timeline-item mt-4 <?php echo $currentStatus === 2 ? 'completed' : ($isCancelled ? 'cancelled' : ''); ?>">
                        <div class="timeline-dot"></div>
                        <p class="font-medium text-gray-900">
                            <?php echo $isCancelled ? ' Đã hủy' : ' Hoàn thành'; ?>
                        </p>
                        <p class="text-sm text-gray-500">
                            <?php echo $isCancelled ? 'Đơn hàng đã bị hủy' : 'Chờ giao thành công'; ?>
                        </p>
                    </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    <a href="../index.php"
                        class="flex-1 py-3 bg-[#FF3F1A] text-white font-semibold rounded-lg text-center hover:bg-red-700 transition">
                        Tiếp tục mua sắm
                    </a>
                    <?php if (isset($_SESSION['user_id']) && $_SESSION['user_id'] == $order['User_id']): ?>
                        <a href="./my-account/orders.php"
                            class="flex-1 py-3 bg-gray-100 text-gray-800 font-semibold rounded-lg text-center hover:bg-gray-200 transition">
                            Xem tất cả đơn hàng
                        </a>
                    <?php endif; ?>
                    <a href="tel:0987879243"
                        class="flex-1 py-3 bg-gray-100 text-gray-800 font-semibold rounded-lg text-center hover:bg-gray-200 transition">
